...
export async function generateMetadata({ params }: { params: Promise<{ platform: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://www.heritageappreals.com/us/services/marketplace/${marketplace.slug}` },
  };
}
...
export default async function USMarketplacePage({ params }: { params: Promise<{ platform: string }> }) {
  ...
  const serviceSchema = {
    ...
    provider: { '@type': 'Organization', name: 'Heritage Apparels', url: 'https://www.heritageappreals.com/us' },
