...
export async function generateMetadata({ params }: { params: Promise<{ platform: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://www.heritageappreals.com/services/marketplace/${marketplace.slug}` },
  };
}
...
export default async function IndiaMarketplacePage({ params }: { params: Promise<{ platform: string }> }) {
  ...
  const serviceSchema = {
    ...
    provider: { '@type': 'LocalBusiness', name: 'Heritage Apparels', url: 'https://www.heritageappreals.com', address: { '@type': 'PostalAddress', addressLocality: 'Mumbai', addressRegion: 'Maharashtra', addressCountry: 'IN' } },
